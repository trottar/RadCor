...
! F1F209.f
! Package of FORTRAN subroutine describing fits to inclusive inelastic
! electron scattering from proton, neutron, deuteron, or heaviere nuclei
! Proton fit is described in:
!   M.E. Christy and P.E. Bosted, ``Empirical Fit to Precision
!    Inclusive Electron-Proton Cross Sections in the Resonance Region'',
!    (arXiv:0712.3731). Submitted to Phys. Rev. C.
! Deuteron (and netron) fit is described in:
!    P.E. Bosted and M.E. Christy, ``Empirical Fit to Inelastic
!    Electron-Deuteron and Electron-Neutron
!    Resonance Region Transverse Cross Sections,
!    (arXiv:0711.0159), publichished in Phys. Rev. C 77, 065206 (2008). (
! New fits for A>2 by Vahe M. and Peter B. (to be publsihed).
!
! routine in this package:
! Main program test. Compiling and linking all code
!    in this package will make a "test" program.
!    Upon execution, the output should be:
!  1.000  1.500  0.617  0.204  0.231  0.312  0.410  1.644  0.072  0.102
!  1.000  2.000  0.472  0.129  0.159  0.292  0.250  2.329  0.199  0.191
!  1.000  2.500  0.382  0.169  0.227  0.417  0.360  2.973  0.241  0.292
!  1.000  3.000  0.321  0.224  0.315  0.511  0.500  3.562  0.327  0.359
!  1.000  3.500  0.276  0.218  0.309  0.517  0.521  4.060  0.348  0.387
!  2.000  1.500  0.763  0.079  0.091  0.110  0.104  0.400  0.154  0.188
!  2.000  2.000  0.641  0.058  0.091  0.157  0.104  0.764  0.184  0.216
!  2.000  2.500  0.553  0.090  0.139  0.241  0.162  1.174  0.215  0.246
!  2.000  3.000  0.485  0.137  0.214  0.324  0.252  1.558  0.233  0.270
!  2.000  3.500  0.433  0.136  0.227  0.354  0.272  1.919  0.267  0.283
!   1.000   0.747   0.432
!   2.000   0.157   0.132
!   3.000   0.048   0.049
!
!
! subroutine F1F2IN09. Returns inelastic F1, F2, and R for
!      nucleus with charge Z and atomic number A
!      for given value of Q2 and W**2. F1 and F2
!      are per nucleus (not per nucleon).
! subroutine pind. Returns F1, R, Sigma_L, and Sigma_T
!      for a proton with Fermi motion of a detueron
! subroutine resd. Returns F1 for a deuteron.
! subroutine resder. Returns error on F1 from fit.
!      Requires auxillary file "F1F207D2emat.dat"
! subroutine resmodd. Returns F1 for average of a free
!      proton and neutron
! subroutine christy507. Returns F1, R, sigma_L, and
!      sigma_T for the free proton.
! subroutine resmod507. Returns sigma_L or sigma_T
!      (called by christy507)
! subroutine mec. Called by resd to get extra terms
!      for dip region between quasi-elastic and Delta
! funtion fitemc. Fit to "EMC effect" used to
!      get F1 and F2 for A>2
!
! subroutine F1F2QE09. Returns quasi-elastic F1, F2 for
!      nucleus with charge Z and atomic number A
!      for given value of Q2 and W**2
!
...
!-----------------------------------------------------------------------
      ...
      real*8 PM/0.938272/, ALPHA/0.007297352568/, HBARC/197.326968/
...
!c        do ism = 1,15
!c          fyuse = fy(ism)
!c          WSQP = WSQ + XX(ISM) * PF * DW2DPF - es * dw2des
        do ism = 1,99
          fyuse = fyp(ism)/100.
...
!c correction to correction Vahe
         if(wsq.gt.0.0) then
      ...
      COMMON/PARCORR/P
!     DATA P/
!    c       5.1141e-02,   9.9343e-01,   5.3317e-02,   1.3949e+00,
!    c       5.9993e+00,  -2.9393e-01,   9.9316e-02,   1.0935e-02,
!    c       3.7697e-01,   1.3542e+00,  -7.4618e+00,   4.3540e+00,
!    c      -3.7911e-01,   3.5105e-01,  -1.8903e+00,   4.9139e+00,
!    c      -5.9923e+00,   2.5021e+00,   1.9943e+01,  -5.5879e-02,
!    c       3.1914e-01,  -1.8657e-01,   3.2746e+01,   4.9801e-03/
       DATA P/
...
!    Electron-Deuteron and Electron-Neutron
!    Resonance Region Transverse Cross Sections,
!    (arXiv:0711.0159). Submitted to Phys. Rev. C.
      implicit none
...
! for given W2, Q2
      ...
      real*8 xval0(12)/
! new 5/07 values. Values 1 and 7 will be overridden below.
...
! make same as resmod507
     ...
     >            kr(i) / k * kcmr(i) / kcm / intwidth(i)
        enddo ! loop on i
!        write(55,'(i5,f7.2,7f10.4)') iw,w,(sigr(iw,i),i=1,7)
       enddo ! loop on iw
...
!cc     &          (1. + q2/0.71)**rescoef(i,4)
! make same as resmod507
     &          (1. + q2/0.91)**rescoef(i,4)
      enddo
!cc      dip = 1./(1. + q2 / 0.71)**2
! make same as resmod507
      dip = 1./(1. + q2 / 0.91)**2
      dip2 = dip**2
!cc      height(7) = xval(49)*dip2
! make same as resmod507
      height(7) = xval(49)*dip
      iw = int(1000.*sqrt(w2))
      sig_res = 0.
      do i=1,7
!cc        sigrsv(i) =  height(i) * sigr(iw,i)
! make same as resmod507 by squaring height
...
! to make same as resmod507
     ...
     &   (nr_coef(i,3)+nr_coef(i,4)*q2+xval(44+i)*q2**2)
      enddo
! new section for sqrt(W)
! make same as resmod507 by turning this off
!cc        sig_nr = sig_nr +(xval(2)*(wdif)**0.5)
!cc     &   /(q2 + xval(3))**
!cc     &   (xval(4) + xval(5) * q2 + xval(6) * q2**2)
...
!     sig_mec = 0.
!     if(wdifp .gt. 0.) then
!       xprp = 1. + (w2p - (md)**2) / (q2 + xval(50))
!       xprp = 1. / xprp
!       sig_mec = (xval(1) + xval(2)*wdifp**(1/2.) +
!    >     xval(3)*wdifp) /
!    &    (q2 + xval(4))**(xval(5) + xval(6) * q2) *
!    >    xprp
!      endif
!     sig = sig_res + sig_nr + sig_mec

      sig = sig_res + sig_nr
!      write(6,'(1x,i4,2f7.2,4e10.3)') iw,q2,w2,height(1),
!     >  sigr(iw,1),sig_res,sig_nr
...
! Fit to proton F1, R, sigma_T, and Sigma_L from
...
!CCC   single pion branching ratios  CCCC
...
!CCC  2-pion branching ratios  CCCC
...
!CCC   Meson angular momentum   CCCC
...
!CC    Calculate kinematics needed for threshold Relativistic B-W  CCC
...
!CC   Calculate partial widths   CCC
...
!CC    End resonance kinematics and Widths calculations   CCC


!CC    Begin resonance Q^2 dependence calculations   CCC
...
!CC    End resonance Q^2 dependence calculations   CCC
...
!CC   Calculate Breit-Wigners for all resonances   CCC
...
!CC    Finish resonances / start non-res background calculation   CCC
