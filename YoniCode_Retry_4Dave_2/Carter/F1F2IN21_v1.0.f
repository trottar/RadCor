C     XZ
C     renamed RESMODP -> RESMODP21; RESMODN -> RESMODN21
C     RESCSP -> RESCSP21; RESCSN -> RESCSN21
C
C     for 3He, calling sfcross for inelastic yields much higher F1, F2 than
C     calling sf for p and n and then adding them together as 2p+n. 
C
CCCCCC
...
c        write(6,*) "in f1f2in21: ", wsq,qsq,eps,wfn,f1,f2
      ...
      elseif(IA.LE.3) then      ! XZ, for 3He (3H works too)
        call sf(WSQ,QSQ,F1p,FLp,F2p,F1n,FLn,F2n)
        F1=IZ*F1p+(IA-IZ)*F1n
        F2=IZ*F2p+(IA-IZ)*F2n
...
CCC   11 GeV kinematic range of Q^2 < 30 GeV^2 and W^2 < 20                 CCC
      ...
      SUBROUTINE rescsp21(W2,Q2,sigT,sigL)
     ...
     & 0.12429E+01,0.00000E+00,0.00000E+00,0.00000E+00,0.10000E-05 /
      ...
      call resmodp21(1,w2,q2,xval1,sigT)
      call resmodp21(2,w2,q2,xvalL,sigL)
...
c      write(6,*) off
      ...
      do i=1,nbins+1      !!!!    First calculate the cross smearing function for each y bin    !!!!
        ...
        call rescsp21(wwz,q2,sigtp,sigLp) 
        call rescsn21(wwz,q2,sigtn,sigLn)
...
CCC///   Read in smearing function array  ///CCC
      ...
      if(firsty) then
c        if(wfn.EQ.1) then
c          open(unit=34,file='f1f2tables/11.WBARELav18',status='old')
c          open(unit=35,file='f1f2tables/f2.WBARELav18',status='old')
c          open(unit=36,file='f1f2tables/f12.WBARELav18',status='old')
c        elseif(wfn.EQ.2) then
...
c        elseif(wfn.EQ.3) then
c          open(unit=34,file='f1f2tables/f11.WBARELwjc1',status='old')        
c          open(unit=35,file='f1f2tables/f2.WBARELwjc1',status='old')
c          open(unit=36,file='f1f2tables/f12.WBARELwjc1',status='old') 
c        elseif(wfn.EQ.4) then
c          open(unit=34,file='f1f2tables/f11.WBARELwjc2',status='old')        
c          open(unit=35,file='f1f2tables/f2.WBARELwjc2',status='old')
c          open(unit=36,file='f1f2tables/f12.WBARELwjc2',status='old')
c        else
c          write(6,*)  "Warning:  wavefunction undefined"
c        endif
...
c      firsty = .true.
      ...
      if(firsty) then
c        if(wfn.EQ.1) then
c          open(unit=37,file='f1f2tables/f11.WBARELav18OFF',status='old')        
c          open(unit=38,file='f1f2tables/f2.WBARELav18OFF',status='old')
c          open(unit=39,file='f1f2tables/f12.WBARELav18OFF',status='old')
c        elseif(wfn.EQ.2) then  !!!!  Not available yet - fix later
c          open(unit=37,file='f1f2tables/f11-offshell-cdbonn.dat',status='old')        
c          open(unit=38,file='f1f2tables/f22-offshell-cdbonn.dat',status='old')
c          open(unit=39,file='f1f2tables/f12-offshell-cdbonn.dat',status='old')
c        elseif(wfn.EQ.3) then
c          open(unit=37,file='f1f2tables/f11.WBARELwjc1OFF',status='old')        
c          open(unit=38,file='f1f2tables/f2.WBARELwjc1OFF',status='old')
c          open(unit=39,file='f1f2tables/f12.WBARELwjc1OFF',status='old') 
c        elseif(wfn.EQ.4) then
c          open(unit=37,file='f1f2tables/f11.WBARELwjc2OFF',status='old')        
c          open(unit=38,file='f1f2tables/f2.WBARELwjc2OFF',status='old')
c          open(unit=39,file='f1f2tables/f12.WBARELwjc2OFF',status='old')
c        else
c          write(6,*)  "Warning:  wavefunction undefined"
c        endif
...
C...Momentum space [qgrid in MeV, uqgrid in MeV^-3/2]
        DO id=1,nq
          READ (ifile,*) qgrid(id), weight(id), uqgrid(id), wqgrid(id)
...
c     OPEN (10,FILE='/u/home/wmelnitc/Work/EMC/D/Wfn/av18.dat',
        ifile=NextUn()
        OPEN (ifile,FILE='f1f2tables/av18.dat',
     &		FORM='FORMATTED')
	DO ip=1,9
          READ (ifile,*)
	ENDDO
        DO ip=1,np
	  READ (ifile,*) Parr(ip), RHOarr(ip)
        ENDDO
        CLOSE (ifile)
        readin = .TRUE.
